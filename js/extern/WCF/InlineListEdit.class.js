...
function InlineListEdit(key, callbackObject) {
	...
	this.init = function() {
		...
		for (var i = 0; i < markAllCheckboxes.length; i++) {
			markAllCheckboxes[i].editor = this;
			markAllCheckboxes[i].onclick = function() { this.editor.markAll(this.checked); }
			markAllCheckboxes[i].checked = markAll;
		}
		
		// mark checkboxes
		for (var id in this.callbackObject.data) {
			...
			if (icon) {
				...
				if (icon.src != undefined) {
					...
					// add hover 
					this.callbackObject.data[id]['icon'] = icon.src;
	...
	this.isMarked = function(id) {
		if (this.callbackObject.data[id]) {
			return this.callbackObject.data[id]['isMarked'];
		}
	...
	this.setMarkedStatus = function(mark, id) {
		if (this.callbackObject.data[id]) {
			if (mark && !this.callbackObject.data[id]['isMarked']) {
				this.callbackObject.data[id]['isMarked'] = 1;
				this.callbackObject.count++;
			}
			else if (!mark && this.callbackObject.data[id]['isMarked']) {
				this.callbackObject.data[id]['isMarked'] = 0;
				this.callbackObject.count--;
			}
	...
	this.areAllMarked = function() {
		if (this.callbackObject.data.length == 0) {
			return false;
		}
		
		for (var id in this.callbackObject.data) {
			if (!this.callbackObject.data[id]['isMarked']) {
				return false;
			}
		}
